...
def handle_tool_error(state) -> dict:
    """
    Process errors when calling tools.
    ...
    """
    error = state.get("error")  # get error info
    tool_calls = state["messages"][-1].tool_calls  #get the tool calls of last message
    return {
        "messages": [
            ToolMessage(
                content=f"Error: {repr(error)}\nPlease correct your error.",
                tool_call_id=tc["id"],  # connect to tool call id with errors
            )
            for tc in tool_calls  # loop all the tool calls
        ]
    }
